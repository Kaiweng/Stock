...
def requests_get(*args1, **args2): #不定長度的給值方法
    i = 3
    while i >= 0:
        try:
...
def crawl_price(date):
    ...
    content = r.text.replace('=', '') #資料中會有=號,把它取代消除
...
def crawl_monthly_report(date):
    print('start crawl_monthly_report')
...
def update_table(conn, table_name, crawl_function, dates):
    ...
    progress = tqdm_notebook(dates, ) #建立進度條物件,以dates數量為準
    
    for d in progress:
        
        print('crawling', d)
        progress.set_description('crawl' + table_name + str(d)) #更新進度條說明
...
def widget(conn, table_name, crawl_func, range_date): #利用呼叫函數建立GUI介面
    date_picker_from = widgets.DatePicker( #建立GUI日期選取物件
        description='from',
        disabled=False,
    )
    
    if table_exist(conn, table_name): #用函數判斷表格是否存在
    ...
    date_picker_to.value = datetime.datetime.now().date() #給現在今天的日期值

    btn = widgets.Button(description='update ') #在看似給值時就建立了GUI物件
    ...
    btn.on_click(onupdate) #觸發條件,呼叫函數
